...
Когда создается новый снимок проекта - коммит, Git обновляет *refs/heads/master* — записывает в него хеш последнего снимка. Получается, что HEAD тоже обновляется, так как ссылается на *refs/heads/master*.
При работе с Git указатель HEAD используется довольно часто. Многие команды Git принимают в качестве параметра хеш снимка. Если нужно передать последний снимок, то вместо его хеша можно просто написать слово HEAD — Git поймёт, что вы имели в виду последний коммит.

## **8. Статусы файлов Git проекта.**

Одна из основных задач Git отслеживать состояние файлов входящих в проект. Основные состояния файлов: *untracked*, *tracked*, *staged*, *modified*.
* untracked - не отслеживается, это новые файлы в репозитории проекта, чтобы отслеживать их состояние необходимо применить к ним команду *git add*;
* staged - файл приобретает это состояние после выполнения команды *git add*, то есть после подготовки к созданию снимака проекта (так же это состяние имеет синонимы - *indexed, cached*);
* tracked - основное состояние всех файлов в репозитории, оно протовополжно состоянию *untracked*, то есть все подготовленные для создания очередного коммита файлы командой *git add* получают это состояние - становятся файлами с отслеживаемым состоянием;
* modified - это состояние файл приобретает любой отслеживаемый файл при внесении в него изменений.

Просмотреть состояние можно командой git status.
Если выполнить команду *git add*, все файлы добавленные этой командой приобретают статус *staged*. Если модифицировать какой либо из файлов проекта он получит статус *staged + modified* если мы не вводили команду *git commit*, и просто *modified* если мы создали очередной снимок перед корректировкой файла. После создания снимака, все добавленные в проект файлы имеют состояние *tracked*. Состояния файлов могут сочетатся, может быть: *untracked, tracked + staged, tracked + staged + modified, tracked + modified, tracked*.

Простая схема изменения состояния файлов:
1. Добавили новый файл в каталог проекта - *untracked*.
2. Добавили новый вайл в репозиторий командой *git add - tracked + staged*.
3. Модифицировали новый файл - *tracked + staged + modified*.
4. Сохранили изменённый файл в репозитории командой *git add - tracked + staged*.
5. Создали новый снимок командой *git commit -m 'Это новый коммит' - tracked*.
6. Модифицировали файл - *tracked + modified*.
7. Сохранили изменения *git add - tracked + staged*.
8. Создаём новый снимок - *tracked*.
9. Ну и дальше в таком же духе...

Графическая схем (русалочная):

```mermaid
graph LR;
 untracked -- " git add " --> staged/tracked ;
 staged/tracked -- " edit file.txt " --> staged/tracked/modified ;
 staged/tracked/modified -- " git add " --> staged/tracked ;
 staged/tracked -- " git commit -m 'message'" --> tracked/commited ;
 tracked/commited -- " edit file.txt " --> tracked/modified ;
 tracked/modified -- " git add " --> staged/tracked ;
```
## **9. Замечания по оформлению коммита.**

Сообщение помещаемое в команду создания снимка/коммита по возможности должно быть информативным и подчинятся определённым правилам.

* Желательно соблюдать длину строки сообщения в 72 символа - именно такое количество выводится по каждому снимку в укороченном логе *git log --oneline*.
* Информация в сообщении должна быть по максимуму конкретезирована и понятна для всех участников проекта.
* В сообщении хорошо указывать тип корректировки - например: *feat* - feature (будующие изменения); *fix* - исправление ошибок.
* Использовать коды задач с которыми оперируют разработчики в команде.
* Прочая короткая и емкая информация делающая понятным назначение конкретного снимака/коммита.
